(function (root) {
  ...
  function makeWeekRotation(nowMs, catalog) {
    ...
    if (!pool.length) throw new Error("CCR TV has no playable music videos in its catalog.");

    const channelSalt = (root.INFINITY_CHANNEL && root.INFINITY_CHANNEL.id) || "CCR-TV";
  ...
  function resolve(nowMs, schedule) {
    if (!Array.isArray(schedule) || !schedule.length) throw new Error("CCR TV schedule is empty.");
