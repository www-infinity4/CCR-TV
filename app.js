(function () {
  ...
  function renderGuide(nowMs) {
    ...
    els.guide.innerHTML = rows.map(item => {
      const p = item.movie;
      return `<article class="guide-row" data-id="${item.id}"><time>${formatTime(item.startsAtMs)}</time><img src="${art(p)}" alt="" loading="lazy"><span><strong>${p.songTitle || p.title}</strong><small>${p.artist || "CCR TV"}</small></span></article>`;
    }).join("");
  }
  ...
  function renderNext(block) {
    ...
    els.next.innerHTML = Array.from({length:6}, (_, n) => schedule[(index + n + 1) % schedule.length]).map(item => {
      const p = item.movie;
      return `<article class="next-card" style="--card-art:url('${art(p).replace(/'/g,"%27")}')"><time>${formatTime(item.startsAtMs)}</time><div><strong>${p.songTitle || p.title}</strong><span>${p.artist || "Roots Rock · Swamp Rock"}</span></div></article>`;
    }).join("");
  }
  ...
  function showIntermission(state, label) {
    ...
    els.cardLabel.textContent = label || "CCR TV INTERMISSION";
  ...
  function tick() {
    ...
    els.artist.textContent = p.artist || "CCR TV";
